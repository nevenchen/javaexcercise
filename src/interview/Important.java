...
public class Important {
	 ...
	 * @Description:条件表达式结果类型的规则：
		...
		另一个的类型是字面常量为0的int型，但0可以表示成char，所以最终返回类型以char类型为准；
	...
	static void ifElseTest(){
		 ...
		 * 但如果将i定义成final时，则返回结果类型为char，则此时符合第二条规则，
             因为final类型的变量在编译时就使用“字面常量0”来替换三元表达式了：
		 */
	 ...
	 *Java语言规范中提到：复合赋值 E1 op= E2等价于简单赋值 E1 = (T)((E1) op (E2))，其中T是E1的类型。
		复合赋值表达式自动地将所执行计算的结果转型为其左侧变量的类型。如果结果的类型与该变量的类型相同，
		那么这个转型不会造成任何影响，然而，如果结果的类型比该变量的类型要宽，那么复合赋值操作符将悄悄地执行一
		个窄化原生类型转换，这样就会导致结果不正确：
		
		使用简单的赋值方式就不会有这样的问题了，因为宽类型不能自动转换成窄的类型，编译器会报错，
		这时我们就会注意到错误：x = x + i;//编译通不过
		请不要将复合赋值操作符作用于byte、short或char类型的变量；在将复合赋值操作符作用于int类型的变量时，
		要确保表达式右侧不是long、float或double类型；在将复合赋值操作符作用于float类型的变量时，
		要确保表达式右侧不是double类型。其实一句：不要将让左侧的类型窄于右侧的数字类型。
		总之，不要在short、byte或char类型的变量之上使用复合赋值操作符，因为这一过程会伴随着计算前类型的提升
		与计算后结果的截断，导致最后的计算结果不正确。
	 * @author: Neven Chen
	 * @param 
	 * @return void
	 * 
